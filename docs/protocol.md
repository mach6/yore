...
1. Resolve the signer's Ed25519 public key. For `register` it is the `sign_key`
   ...
   For the bootstrap `activate` it is the pending signer's own `sign_key`, since
   the first device self-activates while no device is active yet. Otherwise it is
   the `sign_key` of the device named in `X-Yore-Device`, which must be
   **active**.
...
host other than self, page `GET /v1/records?host_id&after&limit` following
`next_after` until caught up. Pull cursors are **persisted** alongside the
downloaded ciphertext in the client's `remote.db`, so a restarting daemon
resumes where it left off rather than re-pulling from `after=0`. That cache is
derived data, and deleting it costs one full re-pull.
...
the stored hash, and safe to publish, since enrolling requires presenting the
...
→ `204`. Idempotent. `400` for a malformed id, `404` if unknown, `409` if
already claimed, since revoking then would say something untrue about how that
machine got in and take nothing away from it. Unlike device revocation this
rotates nothing: the token admitted no one, so there is no key a holder could
already have read with.
...
`GET /v1/recovery/salt` is open, because the salt is an Argon2id input, not a
secret, and is needed before any recovery signature is possible. Everything
after it is signed with the recovery key: `GET /v1/recovery` returns the HK
wrap, `POST /v1/recovery/token` mints an enrollment token, and `POST
/v1/recovery/activate/{id}` admits the replacement machine. Recovery material is
installed by `POST /v1/recovery` at bootstrap and is **write-once**, so a
compromised device cannot swap in a recovery key of its own.
...
**What a revoked machine does.** Rotation stops it reading anything *new*, but
...
the rest of that process. Remote history already decrypted into RAM is
deliberately left for the rest of the session; it is gone at the next start,
which finds an empty cache and can obtain no key. The persisted record is the
durable half: a start that reads it deletes the cache again before opening
anything, covering a process killed mid-purge, a restored backup, or a machine
that comes back up offline and can never be told.
...
in the enrollment path can clear it directly, because the daemon holds the
`data.db` write lock. The only evidence that retires a revocation is the
...
|---|---|---|
| `ping` | none | `ok` (resets the idle timer, nudges ingest) |
| `record` | `record` | `ok` (spools and fsyncs, nudges ingest) |
| `query` | `query` | `query` (`QueryResp`) |
| `hosts` | none | `hosts` (`HostsInfo`) |
| `tags` | `tags` (`TagsReq`: `scope`) | `tags` (`TagsInfo`: known tags with per-tag command counts) |
| `delete` | `delete_id` | `ok` |
| `devices` | none | `devices` (`DevicesInfo`) |
| `token` | none | `token` (`TokenInfo`: a single-use enrollment token) |
| `tokens` | none | `tokens_list` (`TokensInfo`: every token and what became of it) |
...
| `sync` | none | `ok` (runs a full synchronous push/pull cycle) |
| `status` | none | `status` (`StatusResp`) |
| `shutdown` | none | `ok`, then the daemon exits |
...
`ok` false. `RemoteInfo` is `{state, last_sync_ms, hosts}` where `state` is one
of `off | unavailable | syncing | ok`. Pending devices carry a `code`, the same
verification code as enrollment, and `self` marks this machine. </content>
</invoke>
